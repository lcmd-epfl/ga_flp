...
class FrustrationPredictor:
    ...
    def _load_model(self):
        ...
        try:
            ...
            print(f"Frustration model loaded successfully from {self.model_filename}.")
        except (FileNotFoundError, ValueError) as e:
            print(f"Warning: Could not load frustration model from {self.model_filename}. Frustration prediction will be skipped. Error: {e}")
            self.model = None
            self.model_loaded = False
...
def run_frustration_prediction():
    ...
    print(f"Frustration is {predictor.predict_from_xyz(sys.argv[1], idx_b, idx_n)}.")
